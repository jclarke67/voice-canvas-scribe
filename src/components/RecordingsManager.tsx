...
const RecordingsManager: React.FC<RecordingsManagerProps> = ({ isOpen, onClose }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Storage-related errors or others
      if (
        error instanceof DOMException &&
        (error.name === 'QuotaExceededError' || error.name === 'NS_ERROR_DOM_QUOTA_REACHED')
      ) {
        toast.error("Your browser's storage is full. Try deleting old recordings or increase available storage.");
      } else {
        toast.error('Failed to import recording. It may be too large for your browser/device local storage.');
      }
